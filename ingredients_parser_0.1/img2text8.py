...
def preprocess_image(image_path, method='adaptive', display=False, save_path=None):
    ...
    if method == 'adaptive':
        processed = _adaptive_threshold(img)
    elif method == 'contrast':
        processed = _enhance_contrast(img)
    elif method == 'threshold':
        processed = _basic_threshold(img)
    elif method == 'glare':
        processed = _reduce_glare(img)
    elif method == 'morphology':
        processed = _apply_morphology(img)
    elif method == 'multi':
    ...
    if save_path:
        ...
        output_path = os.path.join(os.path.dirname(image_path), f"{name}_processed_{method}{ext}")
...
def compare_all_methods(image_path, display=True, save_dir=None):
    ...
    for method in methods:
        ...
        # Process the image
        processed_path = preprocess_image(image_path, method=method, display=display, save_path=save_path)
...
def test_all_preprocessing_methods(image_path, display=True):
    ...
    for method in methods:
        ...
        print(f"Found {len(processed_ingredients)} ingredients with {method} preprocessing")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Process product label image to extract ingredients')
    parser.add_argument('image_path', type=str, help='Path to the product label image')
    parser.add_argument('--method', type=str, default='test_all', 
                        choices=['adaptive', 'contrast', 'threshold', 'glare', 'morphology', 'multi', 'test_all'],
    ...
    if True:
        ...
        results = test_all_preprocessing_methods(image_path, display=False)
